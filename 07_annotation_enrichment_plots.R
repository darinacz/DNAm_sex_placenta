#####pathway enrichment and annotation for our top hits
library(ggplot2)
library(GenomicRanges)
library(ChIPseeker)
library(TxDb.Hsapiens.UCSC.hg19.knownGene)
require(org.Hs.eg.db)
library(missMethyl)
library(bumphunter)
library(TissueEnrich)
results<-read.table("01_meta/meta/rlm/meta_BET_PREDO_ITU_random1.tbl",sep="\t",header=T)

#####relation to Island => enriched in OpenSea######
#load minfi annotation of EPIC chip
load("annot_epic.Rdata")
#background: all CpGs available in our study
annot<-merge(results,annot_epic,by.x=1,by.y="Name")
index<-which(annot$PvalueARE< 9e-8) #n=10,737 epigenome-wide significant hits

top<-table(annot$Relation_to_Island[index])/10737
#Island    N_Shelf    N_Shore    OpenSea    S_Shelf    S_Shore 
#0.06314613 0.03073484 0.10244947 0.69125454 0.03110739 0.08130763

all<-table(annot$Relation_to_Island[-index])/747364
#Island    N_Shelf    N_Shore    OpenSea    S_Shelf    S_Shore 
#0.19194261 0.03583394 0.09756959 0.55784062 0.03321407 0.08359916 

#plot enrichment 
loc<-rbind(cbind(top, rep("epigenome-wide significant CpGs",6)),cbind(all, rep("non-significant CpGs",6)))
loc<-as.data.frame(loc)
loc$Type<-rownames(loc)
colnames(loc)<-c("Percentage", "Cohort","Type")
loc$Percentage<-as.numeric(loc$Percentage)
loc$RoundedPercentage = round(loc$Percentage, 4)*100
loc$Label = paste(loc$RoundedPercentage,"%", sep="")
loc$Type<-gsub(".1","",loc$Type)
ggplot(loc, aes(x = factor(Cohort), y = Percentage, fill = Type)) + 
  geom_bar(position = position_stack(), stat = "identity", width = .7) +
  geom_text(aes(label = Label), position = position_stack(vjust = 0.5), size = 3) + scale_fill_brewer(palette="RdBu") +ylab("Percentage of CpGs")+xlab("")
dev.print("relation_to_Island_top_CpGs.pdf",dev=pdf)

#enrichment for Open Sea? yes!
table(annot$Relation_to_Island[index])#7422  Open Sea
table(annot$Relation_to_Island[-index])#416910  Open Sea
fisher.test(matrix(c(7422,10737-7422,416910,747364-416910 ),ncol=2,byrow=T))#OR=1.77,p=5.631337e-174

######map to genomic location: enriched for distal intergenic regions#####
index<-which(annot$PvalueARE<9e-08)#n=10,737
top<-annot[index,c("MarkerName","pos","chr")]
top$end<-top$pos+1
top4epic<-makeGRangesFromDataFrame(top,start.field ="pos",end.field="end",seqnames="chr",ignore.strand=T,keep.extra.columns=F)

all<-annot[-index,c("MarkerName","pos","chr")]
all$end<-all$pos+1
all4epic<-makeGRangesFromDataFrame(all,start.field ="pos",end.field="end",seqnames="chr",ignore.strand=T,keep.extra.columns=F)

all_anno<-annotatePeak(all4epic,TxDb=TxDb.Hsapiens.UCSC.hg19.knownGene,annoDb="org.Hs.eg.db",tssRegion = c(-500, 500))
all_anno
#Annotated peaks generated by ChIPseeker
#747364/747364  peaks were annotated
#Genomic Annotation Summary:
#  Feature  Frequency
#9           Promoter 23.2288416
#4             5' UTR  0.7783356
#3             3' UTR  2.9439470
#1           1st Exon  0.7634834
#7         Other Exon  5.2702565
#2         1st Intron 13.9985335
#8       Other Intron 23.0906225
#6 Downstream (<=300)  0.1402262
#5  Distal Intergenic 29.7857537

all_genomic<-c(23.23,0.78,2.94,0.76,5.27,14.00,23.09,0.14,29.79)
all_genomic<-as.data.frame(all_genomic)
all_genomic$position<-c("Promoter","5' UTR","3' UTR", "1st Exon","Other Exon","1st Intron","Other Intron",
                        "Downstream (<=3kb)","Distal Intergenic")


top_anno<-annotatePeak(top4epic,TxDb=TxDb.Hsapiens.UCSC.hg19.knownGene,annoDb="org.Hs.eg.db",tssRegion = c(-500, 500))
top_anno
#Annotated peaks generated by ChIPseeker
#10737/10737  peaks were annotated
#Genomic Annotation Summary:
#  Feature   Frequency
#9           Promoter 12.95520164
#4             5' UTR  0.65195120
#3             3' UTR  2.40290584
#1           1st Exon  0.66126479
#7         Other Exon  3.84651206
#2         1st Intron 15.87035485
#8       Other Intron 23.60994691
#6 Downstream (<=300)  0.07450871
#5  Distal Intergenic 39.92735401

top_genomic<-c(12.96,0.65,2.40,0.66,3.85,15.87,23.61,0.07,39.93)
top_genomic<-as.data.frame(top_genomic)
top_genomic$position<-c("Promoter","5' UTR","3' UTR", "1st Exon","Other Exon","1st Intron","Other Intron",
                        "Downstream (<=3kb)","Distal Intergenic")


loc_genomic<-rbind(cbind(as.numeric(all_genomic[,1]), rep("non-significant CpGs",9),all_genomic[,2]),
                   cbind(as.numeric(top_genomic[,1]), rep("epigenome-wide significant CpGs",9),top_genomic[,2]))
loc_genomic<-as.data.frame(loc_genomic)
colnames(loc_genomic)<-c("Percentage", "Cohort","Type")
loc_genomic$Percentage<-as.numeric(as.character(loc_genomic$Percentage))
loc_genomic$Label = paste(loc_genomic$Percentage,"%", sep="")
loc_genomic$Label[loc_genomic$Percentage <3] <-""

ggplot(loc_genomic, aes(x = factor(Cohort), y = Percentage, fill = Type)) + 
  geom_bar(position = position_stack(), stat = "identity", width = .7) +
  geom_text(aes(label = Label), position = position_stack(vjust = 0.5), size = 3) + scale_fill_brewer(palette="RdBu") +ylab("Percentage of CpGs")+xlab("")
dev.print("location.pdf",dev=pdf)

#top CpGs enriched for distal intergenic?: yes!
length(which(all_anno@detailGenomicAnnotation[,9])) #222608
length(which(top_anno@detailGenomicAnnotation[,9])) #4287
fisher.test(matrix(c(4287,10737-4287,222608,747364 -222608),ncol=2,byrow=T))#OR=1.57,p=4.364642e-109



####pathway enrichment####

#mapping CpGs to nearest genes
genes <- annotateTranscripts(TxDb.Hsapiens.UCSC.hg19.knownGene)

#background genes: all CpGs which are in meta-analysis 
load("annot_epic.Rdata")
annot<-annot_epic[,c(1,2,4,24)]
all<-merge(results,annot,by.x=1, by.y=3)

#map significant genes
#use already created list, this was created for all CpGs on the EPIC array using the bumphunter package
load("genes_CpGs_EPIC.Rdata")
index<-which(all$PvalueARE<9e-08) #n=10,737
top<-all[index,] 
index<-which(epic_genes$CpG %in% top$MarkerName) 
write.table(epic_genes[index,],"significant_genes_all.txt",sep="\t",quote=F,row.names=F)
sig<-unique(epic_genes$name[index])#n=3,775
write.table(sig,"3775_unique_significant_genes.txt",sep="\t",quote=F,row.names=F,col.names=F)

#map all genes
index<-which(epic_genes$CpG %in% all$MarkerName) #n=758,101 = dim(all)[1], ok
all_genes<-epic_genes[index,]
write.table(all_genes,"background_genes_all.txt",sep="\t",quote=F,row.names=F)
genes<-unique(all_genes$name)#n=20,214
write.table(genes,"20214_unique_background_genes.txt",sep="\t",quote=F,row.names=F,col.names=F)

#run FUMA for enrichments (https://fuma.ctglab.nl) using the significant gene and background gene lists
#pathway enrichments
sets<-read.table("FUMA_3775_20214_top_hits/GS.txt",sep="\t",header=T)
go_bp<-sets[sets$Category=="GO_bp",]
go_cc<-sets[sets$Category=="GO_cc",]
go_mf<-sets[sets$Category=="GO_mf",]

go_bp$GeneSet<- factor(go_bp$GeneSet, levels = go_bp$GeneSet[order(-log10(go_bp$p))])
go_bp<-go_bp[order(go_bp$p),]
go_bp$ratio<-go_bp$N_overlap/go_bp$N_genes
#429 at FRD 0.05, plot top25
pdf("GO_enrichment_bp.pdf")
ggplot(go_bp[1:25,], 
       aes(x = -log10(p),y=GeneSet, fill=ratio)) + 
  geom_bar(stat="identity") +
  theme_bw(base_size = 8) +
  ylab(NULL) +
  ggtitle("Top 25 enriched GO biological processes") 
dev.off()

go_cc$GeneSet<- factor(go_cc$GeneSet, levels = go_cc$GeneSet[order(-log10(go_cc$p))])
go_cc<-go_cc[order(go_cc$p),]
go_cc$ratio<-go_cc$N_overlap/go_cc$N_genes
#59 at FRD 0.05, plot top25
pdf("GO_enrichment_cc.pdf")
ggplot(go_cc[1:25,], 
       aes(x = -log10(p),y=GeneSet, fill=ratio)) + 
  geom_bar(stat="identity") +
  theme_bw(base_size = 8) +
  ylab(NULL) +
  ggtitle("Top 25 enriched GO cellular components") 
dev.off()

pdf("GO_enrichment_mf.pdf")
go_mf$GeneSet<- factor(go_mf$GeneSet, levels = go_mf$GeneSet[order(-log10(go_mf$p))])
go_mf<-go_mf[order(go_mf$p),]
go_mf$ratio<-go_mf$N_overlap/go_mf$N_genes
#63 at FRD 0.05, plot top25
ggplot(go_mf[1:25,], 
       aes(x = -log10(p),y=GeneSet, fill=ratio)) + 
  geom_bar(stat="identity") +
  theme_bw(base_size = 8) +
  ylab(NULL) +
  ggtitle("Top 25 enriched GO molecular function") 
dev.off()

#####enrichment for GTEx tissues and TF were also performed using FUMA#####

####tissue enrichment#####

#from Vignette, default: tissueSpecificGeneType = 1, all genes
seEnrichmentOutput<-output[[1]]
enrichmentOutput<-setNames(data.frame(assay(seEnrichmentOutput),
                                      row.names = rowData(seEnrichmentOutput)[,1]),
                           colData(seEnrichmentOutput)[,1])
enrichmentOutput$Tissue<-row.names(enrichmentOutput)
#y=-log10(x) => convert to x via 10^y
enrichmentOutput$p<-10^(-enrichmentOutput$Log10PValue)
enrichmentOutput$FDR<-p.adjust(enrichmentOutput$p,method="BH")
which(enrichmentOutput$FDR<0.05) #thyroid gland,skin,heart muscle, cervix, cerebral cortex

ggplot(enrichmentOutput,aes(x=reorder(Tissue,-Log10PValue),y=Log10PValue,
                            label = Tissue.Specific.Genes,fill = Tissue))+
  geom_bar(stat = 'identity')+
  labs(x='', y = '-LOG10(P-Value)')+
  theme_bw()+
  theme(legend.position='none')+
  theme(plot.title = element_text(hjust = 0.5,size = 20),axis.title =
          element_text(size=15))+
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1),
        panel.grid.major= element_blank(),panel.grid.minor = element_blank())+
  geom_hline(yintercept=-log10(0.003), linetype="dashed", 
             color = "black", size=0.3)
dev.print("enrichment_tissues_HPR_all_genes.pdf",dev=pdf)













